...
constexpr double kdNdEta[kKnownMult]    = {25.75, 19.83, 16.12, 12.78, 10.11, 8.07};
...
void YieldsPlotProton() {
  ...
  for(int iS=0; iS<1; iS++){
    ...
    /// Nucleus mean pt
    TFile mean_pt_file(Form("%s_mean_pt.root",particlename),"recreate");
    TCanvas cMeanPt("cMeanPt","cMeanPt");
    cMeanPt.SetBottomMargin(0.15);
    TGraphErrors mean_pt_gr_stat(kKnownMult,kdNdEta,nucleus_mean_pt,kdNdEtaErr,nucleus_mean_pt_stat);
